...
Приложение сделано в виде супераппа, в котором к функциональности _Личного словаря_ (`PersonalDictionary`) добавлена функциональность _Списка дел_ (`TodoList`). Код разложен по трём модулям, подключаемым к основному приложению (`PersonalDictionary`). Модули реализованы в виде Development Pod'ов CocoaPods.
