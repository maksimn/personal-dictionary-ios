//
//  ToggableItemListView+Layout.swift
//  TodoList
//
...
extension ToggableItemListView {

    func initViews() {
        initCompletedTodoVisibilityToggle()
        initItemListView()
        initCompletedItemCounterView()
    }
    ...
    private func initCompletedItemCounterView() {
        let completedItemCounterView = completedItemCounterGraph.view

        addSubview(completedItemCounterView)
        completedItemCounterView.constraints((self.topAnchor, 16), 20, (self.leadingAnchor, 32), (nil, 0))
    }
}
