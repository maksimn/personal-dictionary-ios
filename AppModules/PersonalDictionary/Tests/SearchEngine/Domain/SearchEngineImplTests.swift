...
final class SearchEngineImplTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        stub(mockSearchableWordList) { stub in
            when(stub.findWords(contain: "b"))
                .thenReturn([word2])
            when(stub.findWords(contain: "d"))
                .thenReturn([])
            when(stub.findWords(whereTranslationContains: "x"))
                .thenReturn([word1, word3])
        }

        searchEngine = SearchEngineImpl(searchableWordList: mockSearchableWordList)
    }
